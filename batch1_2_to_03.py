...
#---------------------------------------------------------------私有特征提取器（有两个并行计算），使用的是一维卷积。对应的是模型图中Conv1d部分------------------------------------------------
        self.cls_fc_son1_2 = nn.Conv1d(128, 64, 3, padding=1)  # (in_channels, out_channels, kernel_size)=(128, 64, 3)
...
#           target_data, __ = target_iter.next()
        ...
        optimizer.zero_grad()
        cls_loss, lmmd_loss, l1_loss, s_fusion1= model(
            source_data, target_data, source_label - 1, mark=1)
...
def train(model, dataloader):
    ...
    with torch.no_grad():                      # 当前计算不需要反向传播
        ...
        print(
            f'Train Average loss: {train_loss:.4f}, Train_Accuracy: {correct}/{(batch_num * batch_size)} ({100. * correct / (batch_num * batch_size):.2f}%)')
        # print(
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        wb = load_workbook(r"data_01/acc-loss-12-03.xlsx")  # 表存在，导入
        ws = wb['Sheet1']
        ...
        wb.save(r"data_01/acc-loss-12-03.xlsx")
    ...
    plot_pca(t_fusion1_cpu, targets_cpu, n_components=2, title='t_fusion1 PCA (2D)',
             save_path_image=f'pca_12_03/t_fusion1_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_12_03/t_fusion1_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(t_fusion2_cpu, targets_cpu, n_components=2, title='t_fusion2 PCA (2D)',
             save_path_image=f'pca_12_03/t_fusion2_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_12_03/t_fusion2_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(s_fusion1_cpu, labels_fusion1_cpu, n_components=2, title='s_fusion1 PCA (2D)',
             save_path_image=f'pca_12_03/s_fusion1_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_12_03/s_fusion1_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(s_fusion2_cpu, labels_fusion2_cpu, n_components=2, title='s_fusion2 PCA (2D)',
             save_path_image=f'pca_12_03/s_fusion2_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_12_03/s_fusion2_pca_2d_epoch_{epoch}.xlsx')
    ...
    plot_tsne(t_fusion1_cpu, targets_cpu, n_components=2, title='t_fusion1 t-SNE (2D)',
              save_path_image=f'pca_12_03/t_fusion1_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_12_03/t_fusion1_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(t_fusion2_cpu, targets_cpu, n_components=2, title='t_fusion2 t-SNE (2D)',
              save_path_image=f'pca_12_03/t_fusion2_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_12_03/t_fusion2_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(s_fusion1_cpu, labels_fusion1_cpu, n_components=2, title='s_fusion1 t-SNE (2D)',
              save_path_image=f'pca_12_03/s_fusion1_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_12_03/s_fusion1_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(s_fusion2_cpu, labels_fusion2_cpu, n_components=2, title='s_fusion2 t-SNE (2D)',
              save_path_image=f'pca_12_03/s_fusion2_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_12_03/s_fusion2_tsne_2d_epoch_{epoch}.xlsx')
