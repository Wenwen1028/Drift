...
where $V_{chamber}$ and $\gamma$ denote the volume of the chamber, with a specific value of 35 L, and the proportion coefficient of the target gas in the cylinder, respectively. \par  
3) Gas sensor array signal collection stage: employ a syringe to withdraw the target gas from the gas collection bag and introduce it into the gas chamber. Simultaneously, activate the chamber fan for purging  and initiate the LabVIEW interface to  record the dynamic response signal for approximately 350 s, denoted as $T$.   Subsequently, close the LabVIEW interface. \par
4) Iterate through stage 1 to 3 until all desired gas dynamic response signals have been collected. \par 
# Getting Started
## Installation
