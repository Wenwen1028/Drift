...
def TFN_fusion (A, B):
    ...
    fusion_AB = fusion_AB.flatten(start_dim=1)
...
class TransAm(nn.Module):
    def __init__(self, feature_size=128, num_layers=1, dropout=0.01):
...
#           target_data, __ = target_iter.next()
        ...
        lambd = 2 / (1 + math.exp(-0.0008 * (epoch) / epochs)) - 1
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        wb = load_workbook(r"data_01/acc-loss-12-05.xlsx")  # 表存在，导入
        ws = wb['Sheet1']
        ...
        wb.save(r"data_01/acc-loss-12-05.xlsx")
    ...
    torch.save(model.state_dict(), 'trained_models/' + tgt_name + '.pt')
         # 将我们所需要的数据保存在文件里面供可视化使用；第一个参数可以简单理解为保存图的名称，第二个参数是可以理解为Y轴数据，第三个参数可以理解为X轴数据
    ...
    plot_pca(t_fusion1_cpu, targets_cpu, n_components=2, title='t_fusion1 PCA (2D)',
             save_path_image=f'pca_12_05/t_fusion1_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_12_05/t_fusion1_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(t_fusion2_cpu, targets_cpu, n_components=2, title='t_fusion2 PCA (2D)',
             save_path_image=f'pca_12_05/t_fusion2_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_12_05/t_fusion2_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(s_fusion1_cpu, labels_fusion1_cpu, n_components=2, title='s_fusion1 PCA (2D)',
             save_path_image = f'pca_12_05/s_fusion1_pca_2d_epoch_{epoch}.png',
             save_path_excel = f'pca_12_05/s_fusion1_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(s_fusion2_cpu,  labels_fusion2_cpu, n_components=2, title='s_fusion2 PCA (2D)',
             save_path_image = f'pca_12_05/s_fusion2_pca_2d_epoch_{epoch}.png',
             save_path_excel = f'pca_12_05/s_fusion2_pca_2d_epoch_{epoch}.xlsx')
    ...
    plot_tsne(t_fusion1_cpu, targets_cpu, n_components=2, title='t_fusion1 t-SNE (2D)',
               save_path_image=f'pca_12_05/t_fusion1_tsne_2d_epoch_{epoch}.png',
               save_path_excel=f'pca_12_05/t_fusion1_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(t_fusion2_cpu, targets_cpu, n_components=2, title='t_fusion2 t-SNE (2D)',
               save_path_image=f'pca_12_05/t_fusion2_tsne_2d_epoch_{epoch}.png',
               save_path_excel=f'pca_12_05/t_fusion2_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(s_fusion1_cpu, labels_fusion1_cpu, n_components=2, title='s_fusion1 t-SNE (2D)',
               save_path_image=f'pca_12_05/s_fusion1_tsne_2d_epoch_{epoch}.png',
               save_path_excel=f'pca_12_05/s_fusion1_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(s_fusion2_cpu, labels_fusion2_cpu, n_components=2, title='s_fusion2 t-SNE (2D)',
               save_path_image=f'pca_12_05/s_fusion2_tsne_2d_epoch_{epoch}.png',
               save_path_excel=f'pca_12_05/s_fusion2_tsne_2d_epoch_{epoch}.xlsx')
