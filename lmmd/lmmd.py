...
class LMMD_loss(nn.Module):
    def __init__(self, class_num=6, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    ...
    def convert_to_onehot(self, sca_label, class_num=6):
        return np.eye(class_num)[sca_label]

    def cal_weight(self, s_label, t_label, batch_size=32, class_num=6):
    ...
    def mmd_rbf_noaccelerate(self,source, target, kernel_mul=2.0, kernel_num=3, fix_sigma=None):
    ...
    def cmmd(self, source, target, s_label, t_label):
        s_label = s_label.cpu()
        s_label = s_label.view(5, 1)
        s_label = torch.zeros(5,6).scatter_(1, s_label.data, 1)
        s_label = Variable(s_label).cuda()

        t_label = t_label.cpu()
        t_label = t_label.view(5, 1)
        t_label = torch.zeros(5, 6).scatter_(1, t_label.data, 1)
