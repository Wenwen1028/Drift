...
class TransAm(nn.Module):
    def __init__(self, feature_size=40, num_layers=3, dropout=0.2):
        ...
        self.decoder_1 = nn.Linear(feature_size, 32)
...
class Res_TransAm1(nn.Module):
    def __init__(self, feature_size=40, num_layers=2, dropout=0.2):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=feature_size, nhead=20, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=num_layers)
        self.decoder_1 = nn.Linear(feature_size, 16)
...
#----------------------------------------------------------私有特征提取器（有两个并行计算），使用的是一维卷积。对应的是模型图中Conv1d部分------------------------------------------------
        self.cls_fc_son1_2 = nn.Conv1d(32, 16, 3, padding=1)
        self.cls_fc_son2_2 = nn.Conv1d(32, 16, 3, padding=1)
...
#-----------------------------------------------------------------接在私有特征叠加融合后进行降维与分类训练。对应的是模型图中的C1和C2部分-------------------------------------------------
        self.cls_fc1_1 = nn.Linear(16, 10)
        self.cls_fc2_1 = nn.Linear(16, 10)
        self.cls_fc1_2 = nn.Linear(10, num_classes)
        self.cls_fc2_2 = nn.Linear(10, num_classes)
...
# ---------------------------------------------------------------------------源域1和目标域进行计算与对齐-----------------------------------------------------------------------
        if mark == 1:
            source = source.unsqueeze(0)                  # 扩充一个维度 (1, 8, 40)
            s_fea = (self.dense(source))                  # 源领域进入common features extractor (1, 8, 20)
            source = source.squeeze()                     # (8, 40)
            s_fea = s_fea.permute(1, 2, 0)                # (8, 16, 1)

            s_fea2 = self.cls_fc_son1_2(s_fea)            # 一维卷积 (8, 16, 1)     (batch_size, seq_length, embedding_dim)
            s_fea2 = s_fea2.permute(2, 0, 1)              #  （1, 8, 16）
            s_fea2 = s_fea2.squeeze(0)                    # （8, 16）
            s_fea2 = s_fea2.reshape(-1, 4, 4)             # (8, 4, 4)
            s_fea2 = s_fea2.unsqueeze(1)                  # (8, 1, 4, 4)

            source = source.unsqueeze(0)                  # (1, 8, 40)
            s_res = self.res1(source)                     # (1, 8, 16)
            s_res_l = s_res.squeeze()                     # (8, 16)
            s_res = s_res_l.reshape(-1, 4, 4)             # (8, 4, 4)
            s_res = s_res.unsqueeze(1)                    # (8, 1, 4, 4)
            s_fusion = self.fusion(s_fea2, s_res)         #
            s_fusion1 = s_fusion.squeeze(1).reshape(-1, 16)
            ...
            t2_fea2 = t2_fea2.reshape(-1, 4, 4)              # (24, 5, 10)
            t2_fea2 = t2_fea2.unsqueeze(1)                    # (24, 1, 5, 10)
            target = target.unsqueeze(0)
            t_res = self.res2(target)
            t_res = t_res.squeeze()
            t_res = t_res.reshape(-1, 4, 4)              # (24, 5, 10)
            t_res = t_res.unsqueeze(1)                    # (24, 1, 5, 10)
...
# ---------------------------------------------------------------------------源域2和目标域进行计算与对齐-----------------------------------------------------------------------
        if mark == 2:
            ...
            s_fea2 = s_fea2.reshape(-1, 4, 4)  # (24, 5, 10)
            s_fea2 = s_fea2.unsqueeze(1)        # (24, 1, 5, 10)
            source = source.unsqueeze(0)
            s_res = self.res2(source)
            s_res_l = s_res.squeeze()
            s_res = s_res_l.reshape(-1, 4, 4)  # (24, 5, 10)
            s_res = s_res.unsqueeze(1)
            s_fusion = self.fusion(s_fea2, s_res)  #
            s_fusion2 = s_fusion.squeeze(1).reshape(-1, 16)
    ...
    def predict(self, source):
        ...
        s_res = s_res.squeeze()
        s_res = s_res.reshape(-1, 4, 4)    # (24, 5, 10)
        s_res = s_res.unsqueeze(1)         # (24, 1, 5, 10)
        s_fusion = self.fusion(s_fea2, s_res)  #
        t_fusion1 = s_fusion.squeeze(1).reshape(-1, 16)
...
#           target_data, __ = target_iter.next()
        ...
        lambd = 2 / (1 + math.exp(-5 * (epoch) / epochs)) - 1
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        wb = load_workbook(r"data_01/ex_acc-loss-1112-14.xlsx")  # 表存在，导入
        ws = wb['Sheet1']
        ...
        wb.save(r"data_01/ex_acc-loss-1112-14.xlsx")
    ...
    plot_pca(t_fusion1_cpu, targets_cpu, n_components=2, title='t_fusion1 PCA (2D)',
             save_path_image=f'pca_1112_14/t_fusion1_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_1112_14/t_fusion1_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(t_fusion2_cpu, targets_cpu, n_components=2, title='t_fusion2 PCA (2D)',
             save_path_image=f'pca_1112_14/t_fusion2_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_1112_14/t_fusion2_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(s_fusion1_cpu, labels_fusion1_cpu, n_components=2, title='s_fusion1 PCA (2D)',
             save_path_image=f'pca_1112_14/s_fusion1_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_1112_14/s_fusion1_pca_2d_epoch_{epoch}.xlsx')
    plot_pca(s_fusion2_cpu, labels_fusion2_cpu, n_components=2, title='s_fusion2 PCA (2D)',
             save_path_image=f'pca_1112_14/s_fusion2_pca_2d_epoch_{epoch}.png',
             save_path_excel=f'pca_1112_14/s_fusion2_pca_2d_epoch_{epoch}.xlsx')
    ...
    plot_tsne(t_fusion1_cpu, targets_cpu, n_components=2, title='t_fusion1 t-SNE (2D)',
              save_path_image=f'pca_1112_14/t_fusion1_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_1112_14/t_fusion1_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(t_fusion2_cpu, targets_cpu, n_components=2, title='t_fusion2 t-SNE (2D)',
              save_path_image=f'pca_1112_14/t_fusion2_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_1112_14/t_fusion2_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(s_fusion1_cpu, labels_fusion1_cpu, n_components=2, title='s_fusion1 t-SNE (2D)',
              save_path_image=f'pca_1112_14/s_fusion1_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_1112_14/s_fusion1_tsne_2d_epoch_{epoch}.xlsx')
    plot_tsne(s_fusion2_cpu, labels_fusion2_cpu, n_components=2, title='s_fusion2 t-SNE (2D)',
              save_path_image=f'pca_1112_14/s_fusion2_tsne_2d_epoch_{epoch}.png',
              save_path_excel=f'pca_1112_14/s_fusion2_tsne_2d_epoch_{epoch}.xlsx')
